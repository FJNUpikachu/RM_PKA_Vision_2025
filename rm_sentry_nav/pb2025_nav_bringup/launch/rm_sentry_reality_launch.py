...
def generate_launch_description():
    ...
    get_package_share_directory('pb2025_nav_bringup'), 'urdf', 'pka_sentry.urdf.xacro')])

    robot_gimbal_description = Command(['xacro ', os.path.join(
    get_package_share_directory('pb2025_nav_bringup'), 'urdf', 'rm_gimbal.urdf.xacro')])
    ...
    declare_mapping_cmd = DeclareLaunchArgument(
        "mapping", default_value="False", description="Whether start mapping"
    )
    ...
    ld.add_action(robot_navigation_publisher)
